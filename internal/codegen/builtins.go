...
func (cg *CodeGen) defineBuiltins() {
	cg.defineMkStr()
	cg.defineStrlen()
}
...
func (cg *CodeGen) defineMkStr() {
	// strParam := ir.NewParam("size", types.I64)
	// flint_strlen := cg.mod.NewFunc("flint_mkstr", types.I8Ptr, strParam)

	// entry := flint_strlen.NewBlock("entry")
	// entry.NewUnreachable()

	// cg.funcs["mkstr"] = flint_strlen
}
